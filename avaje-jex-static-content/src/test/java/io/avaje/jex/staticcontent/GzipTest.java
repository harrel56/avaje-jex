...
class GzipTest {
  ...
  void normalGzip() throws IOException {
    TestPair pair = TestPair.create(Jex.create()
      .plugin(StaticContent.ofClassPath("/public").route("/index").directoryIndex("index.html").build()));

    HttpResponse<InputStream> res = pair.request().header("Accept-Encoding", "gzip").path("index").GET().asInputStream();
  ...
  void precompressGzip() throws IOException {
    TestPair pair = TestPair.create(Jex.create()
      .plugin(StaticContent.ofClassPath("/public").route("/index").directoryIndex("index.html").preCompress().build()));
